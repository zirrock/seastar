...
BOOST_AUTO_TEST_CASE(kafka_produce_response_parsing_test) {
    ...
    BOOST_REQUIRE_EQUAL(response.get_responses()->size(), 1);
    BOOST_REQUIRE_EQUAL(*response.get_throttle_time_ms(), 0);

    const auto &inner_response = response.get_responses()[0];
    BOOST_REQUIRE_EQUAL(*inner_response.get_name(), "test5");
    BOOST_REQUIRE_EQUAL(inner_response.get_partitions()->size(), 1);

    const auto &partition = inner_response.get_partitions()[0];
    BOOST_REQUIRE_EQUAL(*partition.get_partition_index(), 0);
    BOOST_REQUIRE_EQUAL(*partition.get_error_code(), 0);
    BOOST_REQUIRE_EQUAL(*partition.get_base_offset(), 0x46);
    BOOST_REQUIRE_EQUAL(*partition.get_log_append_time_ms(), -1);
    BOOST_REQUIRE_EQUAL(*partition.get_log_start_offset(), 0);
}
