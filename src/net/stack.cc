...
future<connected_socket_and_address> server_socket::accept() {
    if (_aborted) {
        return make_exception_future<connected_socket_and_address>(std::system_error(ECONNABORTED, std::system_category()));
    }
    return _ssi->accept();
}
