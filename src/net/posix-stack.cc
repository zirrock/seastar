...
template <transport Transport>
future<connected_socket_and_address>
posix_server_socket_impl<Transport>::accept() {
    return _lfd.accept().then([this] (pollable_fd fd, socket_address sa) {
        ...
        if (cpu == engine().cpu_id()) {
            ...
            return make_ready_future<connected_socket_and_address>(connected_socket_and_address{connected_socket(std::move(csi)), sa});
        } else {
...
template <transport Transport>
future<connected_socket_and_address> posix_ap_server_socket_impl<Transport>::accept() {
    ...
    if (conni != conn_q.end()) {
        ...
        try {
            ...
            return make_ready_future<connected_socket_and_address>(connected_socket_and_address{connected_socket(std::move(csi)), std::move(c.addr)});
        } catch (...) {
            return make_exception_future<connected_socket_and_address>(std::current_exception());
        }
    } else {
        try {
...
posix_ap_server_socket_impl<Transport>::move_connected_socket(socket_address sa, pollable_fd fd, socket_address addr, conntrack::handle cth, compat::polymorphic_allocator<char>* allocator) {
    ...
    if (i != sockets.end()) {
        try {
            ...
            i->second.set_value(connected_socket_and_address{connected_socket(std::move(csi)), std::move(addr)});
        } catch (...) {
...
template<transport Transport>
thread_local std::unordered_map<socket_address, promise<connected_socket_and_address>> posix_ap_server_socket_impl<Transport>::sockets;
template<transport Transport>
